...
Per-parameter descriptions extracted from the `JsonSchema` properties bag in the enclosing fn (`tools/src/agent_tool.rs:251`). The model sees each `description` field on each parameter at tool-spec time.
